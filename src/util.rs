...
/// Concatenates a url with a vector or parameter=value(s)
///
/// Example:
/// concat_url(
...
pub fn concat_url(url: String, rest: Vec<String>) -> String {
    ...
    return vec![url, first_mark.to_string(), rest.join("&")].join("");
}

/// Checks if a vector of T contais all Ts in content
pub fn contains_all<T: std::cmp::PartialEq>(container: &Vec<T>, content: &Vec<T>) -> bool {
...
pub fn fail_with_log<T>(message: &str) -> Result<T, Box<dyn Error>> {
